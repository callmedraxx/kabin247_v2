...
<div class="tots"><div class="tots-box"><div class="tot-row"><span>Subtotal:</span><span>$${formatPrice(order.subtotal)}</span></div>${order.delivery_fee>0?`<div class="tot-row"><span>${escapeHtml(airportCode)} Delivery Fee:</span><span>$${formatPrice(order.delivery_fee)}</span></div>`:''}${order.service_charge>0?`<div class="tot-row"><span>Service Charge:</span><span>$${formatPrice(order.service_charge)}</span></div>`:''}${order.coordination_fee>0?`<div class="tot-row"><span>Coordination Fee:</span><span>$${formatPrice(order.coordination_fee)}</span></div>`:''}${order.airport_fee>0?`<div class="tot-row"><span>Airport Fee:</span><span>$${formatPrice(order.airport_fee)}</span></div>`:''}${order.fbo_fee>0?`<div class="tot-row"><span>FBO Fee:</span><span>$${formatPrice(order.fbo_fee)}</span></div>`:''}${order.shopping_fee>0?`<div class="tot-row"><span>Shopping Fee:</span><span>$${formatPrice(order.shopping_fee)}</span></div>`:''}${order.restaurant_pickup_fee>0?`<div class="tot-row"><span>Restaurant Pickup Fee:</span><span>$${formatPrice(order.restaurant_pickup_fee)}</span></div>`:''}${order.airport_pickup_fee>0?`<div class="tot-row"><span>Airport Pickup Fee:</span><span>$${formatPrice(order.airport_pickup_fee)}</span></div>`:''}<div class="tot-row grand"><span>Total:</span><span>$${formatPrice(order.total)}</span></div></div></div>
...
export function generateOrderPDF(order: any, styles: PDFStyleConfig = defaultPDFStyles): typeof PDFDocument.prototype {
  ...
  if (order.coordination_fee > 0) { doc.text('Coordination Fee:', totX + 15, ty).text(`$${formatPrice(order.coordination_fee)}`, totX + 15, ty, { width: totW - 30, align: 'right' }); ty += 20; }
  if (order.airport_fee > 0) { doc.text('Airport Fee:', totX + 15, ty).text(`$${formatPrice(order.airport_fee)}`, totX + 15, ty, { width: totW - 30, align: 'right' }); ty += 20; }
  if (order.fbo_fee > 0) { doc.text('FBO Fee:', totX + 15, ty).text(`$${formatPrice(order.fbo_fee)}`, totX + 15, ty, { width: totW - 30, align: 'right' }); ty += 20; }
  if (order.shopping_fee > 0) { doc.text('Shopping Fee:', totX + 15, ty).text(`$${formatPrice(order.shopping_fee)}`, totX + 15, ty, { width: totW - 30, align: 'right' }); ty += 20; }
  if (order.restaurant_pickup_fee > 0) { doc.text('Restaurant Pickup Fee:', totX + 15, ty).text(`$${formatPrice(order.restaurant_pickup_fee)}`, totX + 15, ty, { width: totW - 30, align: 'right' }); ty += 20; }
  if (order.airport_pickup_fee > 0) { doc.text('Airport Pickup Fee:', totX + 15, ty).text(`$${formatPrice(order.airport_pickup_fee)}`, totX + 15, ty, { width: totW - 30, align: 'right' }); ty += 20; }
  ty += 6;
